...
import { executeProcessAdapter } from "../../../src/adapters/process/execute.js";
...
describe("executeProcessAdapter", () => {
  it("executes echo and captures output", async () => {
    const result = await executeProcessAdapter(makeCtx({
  ...
  it("calls onOutput callback", async () => {
    ...
    await executeProcessAdapter(makeCtx({
